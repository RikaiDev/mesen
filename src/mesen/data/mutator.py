...
def mutate_html(
    ...
    if mutation_type == MutationType.OVERFLOW:
        ...
        unbroken_text = (
            "SupercalifragilisticexpialidociousUnbrokenContainerBreakerTextWithoutSpaces" * 3
        )
        mutated_html = base_html.replace("</h1>", f" - {unbroken_text}</h1>").replace(
            "</h2>", f" - {unbroken_text}</h2>"
        )
